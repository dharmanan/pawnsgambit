...
const App: React.FC = () => {
    ...
    const getInitialBoard = () => {
        ...
        return currentLevel.layout.map((row: number[], r: number) =>
    ...
    const initializeBoard = useCallback((levelToLoad: number) => {
        ...
        const newBoard = currentLevel.layout.map((row: number[], r: number) =>
            row.map((cell: number, c: number) => {
                if (cell === 1) {
    ...
    const makeMove = useCallback((from: Position, to: Position) => {
        ...
        setTimeout(() => {
            const newBoard = board.map((row: (null | { hasPeg: boolean })[]) => row.map((cell: null | { hasPeg: boolean }) => cell ? { ...cell } : null));
    ...
    const highestUnlockedLevel = useMemo(() => {
        ...
        for (let i = 1; i < LEVELS.length; i++) {
            ...
            const hasRequiredBadge = levelAchievements.some((badgeIdentifier: string) => requiredBadgeIdentifiers.has(badgeIdentifier));
            if (hasRequiredBadge) {
                maxLevel = i + 1;
            } else {
                break; 
    ...
    return (
        <div className="text-white min-h-screen font-sans p-4">
            ...
            {/* Sadece en üstte ortalanmış cüzdan bağla butonu */}
            ...
            <main className="flex flex-col items-center">
                ...
                {/* GameOverModal'ı <main> içinde, en sonda göster */}
